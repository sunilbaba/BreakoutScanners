"""
nifty_engine_gh.py
THE "UNIFIED" INSTITUTIONAL ENGINE
----------------------------------
Integrates:
1. AI Strategy Loading (RSI/BB/MACD/Breakout).
2. Market Regime & Relative Strength Filtering.
3. 5-Year Asset-Specific Validation (Deep Dive).
4. Realistic Portfolio Management (Gaps, Costs, TSL).
5. Full HTML Dashboard Generation.
"""
...
CAPITAL = 100000.0
RISK_PER_TRADE = 0.02  
DATA_PERIOD = "2y"     # Needed for 200 SMA & RS Calc
DEEP_PERIOD = "5y"     # For deep validation
...
BROKERAGE_PCT = 0.001 
TSL_MOVE_AT = 0.5 # Move SL to BE at 50% to target
...
def prepare_df(df):
    ...
    # Relative Strength (6M ROC)
    df['ROC_6M'] = df['Close'].pct_change(126) * 100
    
    # Breakout High (20 Day)
    df['HIGH_20'] = df['High'].rolling(20).max().shift(1)

    return df

# --- 5. DEEP DIVE VALIDATOR ---
def run_deep_dive(ticker):
    """
    Downloads 5 years of data for ONE stock to validate the current AI strategy on it.
    Returns: Win Rate, Trade Count, and Full Trade Log.
    """
    ...
    sma200 = df['SMA200'].values; rsi = df['RSI'].values; adx = df['ADX'].values
    atr = df['ATR'].values; bb_low = df['BB_LOW'].values; macd = df['MACD'].values
    macds = df['MACD_SIG'].values; dates = df.index
    ...
    while i < end:
        ...
        elif st == "BREAKOUT" and close[i] >= df['HIGH_20'].iloc[i]: is_entry = True
        
        if is_entry:
            sl = close[i] - (sl_m * atr[i])
            tgt = close[i] + (tgt_m * atr[i])
            outcome = "OPEN"; exit_date = "-"; pnl_pct = 0.0
            ...
            if outcome != "OPEN":
                trade_log.append({ "date": dates[i].strftime("%Y-%m-%d"), "exit": exit_date, "result": outcome, "pnl": pnl_pct })
            i += days
        else: i += 1
...
def analyze_ticker(ticker, df, benchmark_roc, market_regime):
    ...
    # 1. Relative Strength Check (Mirroring the Training)
    ...
    if st_type == "RSI_DIP":
        ...
        if float(prev['MACD']) < float(prev['MACD_SIG']) and float(curr['MACD']) > float(curr['MACD_SIG']): is_entry = True; setup_name = "MACD Cross"
         ...
         if close > float(df['HIGH_20'].iloc[-1]): is_entry = True; setup_name = "Breakout"

    verdict = "WAIT"; v_color = "gray"
    deep_wr = 0; deep_trades = 0; deep_log = []
    
    if trend_ok and is_entry:
        # 2. 5-Year Validation (Deep Dive)
...
def update_trades(trades, bulk_data):
    ...
    for t in trades:
        if t['status'] == 'OPEN':
            # Auto-Enrich Active Trade with Deep Dive if missing
            if 'deep_dive' not in t:
            ...
            if df is not None:
                curr = df.iloc[-1]
                low, high, close = float(curr['Low']), float(curr['High']), float(curr['Close'])
                open_p = float(curr['Open'])
                
                t['current_price'] = round(close, 2)
                t['current_pnl'] = round((close - t['entry']) * t['qty'], 2)
                
                # Exit Logic with Gap Protection
                status = t['status']
                exit_p = None
                
                if low <= t['stop_loss']: 
                    status = 'LOSS'
                    exit_p = open_p if open_p < t['stop_loss'] else t['stop_loss'] # Gap Down Check
                elif high >= t['target']:
                    status = 'WIN'
                    exit_p = open_p if open_p > t['target'] else t['target'] # Gap Up Check
                else:
                    # TSL Check (Move to BE at 50% progress)
                    move_needed = (t['target'] - t['entry']) * TSL_MOVE_AT
                    if close > (t['entry'] + move_needed) and t['stop_loss'] < t['entry']:
...
def generate_html(signals, trades, bt_stats, timestamp, regime):
    ...
    <body class="bg-slate-900 text-slate-200 p-4">
        ...
        <div id="stockModal" class="fixed inset-0 bg-black/80 hidden flex items-center justify-center z-50" onclick="this.classList.add('hidden')">
            <div class="bg-slate-800 p-6 rounded-lg border border-slate-600 w-full max-w-2xl h-3/4 overflow-hidden flex flex-col" onclick="event.stopPropagation()">
                <div class="flex justify-between items-center mb-4">
                    <div><h2 id="m-sym" class="text-2xl font-bold text-white">SYMBOL</h2><div class="text-xs text-slate-400">5-Year Strategy Performance</div></div>
        ...
        <script>
            ...
            function setTab(id) {{ ['dash', 'ledger'].forEach(t => {{ document.getElementById('view-'+t).classList.add('hidden'); document.getElementById('btn-'+t).className="font-bold text-slate-500 pb-2"; }}); document.getElementById('view-'+id).classList.remove('hidden'); document.getElementById('btn-'+id).className="font-bold text-white border-b-2 border-purple-500 pb-2"; }}
            ...
            function showModal(idx, type) {{
                ...
                document.getElementById('m-hist-body').innerHTML = stats.log.length ? stats.log.map(l => `<tr class="border-b border-slate-800"><td class="p-2 text-slate-400">${{l.date}}</td><td class="p-2 font-bold ${{l.result==='WIN'?'text-green-400':'text-red-400'}}">${{l.result}}</td><td class="p-2">${{l.pnl}}%</td></tr>`).join('') : '<tr><td colspan="3" class="p-4 text-center text-slate-600">No history found.</td></tr>';
...
if __name__ == "__main__":
    ...
    for t in cols:
        ...
        try:
            res = analyze_ticker(t, extract_df(bulk, t), nifty_roc, regime)
